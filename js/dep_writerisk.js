function DepWriteRisk (content,id=null) {
        $(content).empty().append("<h2 style='color: blue'>ความเสี่ยงที่งานรายงาน</h2>"+
                                    "<ol class='breadcrumb'>"+
                                    "<li><a href='index.html'><i class='fa fa-home'></i> หน้าหลัก</a></li>"+
                                    "<li class='active'><i class='fa fa-envelope'></i> ความเสี่ยงที่งานรายงาน</li>"+
                                    "</ol>"+
                                    ...
                                    "<h4 class='box-title'><i class='fa fa-star'></i> ความเสี่ยงที่งานของเรารายงาน </h4></div>"+
    ...
    //var column1 = '{"รายการความเสี่ยงที่รอ RM man มาตรวจสอบ":["เลขที่","รายการ","เกิดขึ้นเมื่อ","ได้รับเมื่อ"]}';
            ...
            if(idsymp == null){
              var CTb = new createTableAjax();
                  CTb.GetNewTableAjax('contentTB','JsonData/DT_depR2.php?'+$("#yearS").val(),'JsonData/tempSendData.php',column1
             ...
             var CTb = new createTableAjax();
                  CTb.GetNewTableAjax('contentTB','JsonData/DT_depR2.php?'+$.cookie('year')+'?'+idsymp.data,'JsonData/tempSendData.php',column1
              ,null,null,null,null,false,true,'DetailRisk',false,null,false,null,'ไม่ผ่าน',null,'กำลังดำเนินการ','ผ่านการทบทวน','dbtb');
              $("select#yearS").change(function () {
                  $("#Budget").empty().append("ปีงบประมาณ : "+$("#yearS").val());
                  CTb.GetNewTableAjax('contentTB','JsonData/DT_depR2.php?'+$("#yearS").val()+'?'+idsymp.data,'JsonData/tempSendData.php',column1
              ,null,null,null,null,false,true,'DetailRisk',false,null,false,null,'ไม่ผ่าน',null,'กำลังดำเนินการ','ผ่านการทบทวน','dbtb');
              }); 
          }
        }
